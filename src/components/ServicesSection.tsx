...
const HolographicIcon = React.memo(
  ({
    ...
    return (
      ...
      >
        <div
          className={`p-2 sm:p-3 rounded-xl bg-gradient-to-r ${gradient} ${shouldReduceMotion ? "" : "group-hover:scale-110"} transition-transform duration-200`}
        >
          <IconComponent className='w-5 h-5 sm:w-6 sm:h-6 text-[#111316]' />
        </div>
        {!shouldReduceMotion && (
          <motion.div
            className={`absolute inset-0 rounded-xl bg-gradient-to-r ${gradient} opacity-20 blur-lg`}
  ...
  ({ service, gradient }: { service: Service; gradient: string }) => {
    ...
    return (
      <motion.div
        ...
        className='group/service flex items-center gap-3 sm:gap-4 p-3 sm:p-4 rounded-2xl bg-white/5 border border-white/10 hover:bg-white/10 hover:border-white/20 transition-all duration-200 backdrop-blur-xl'
      >
        <div className='flex-shrink-0'>
          <div
            className={`p-2 rounded-lg sm:rounded-xl bg-gradient-to-r ${gradient} ${shouldReduceMotion ? "" : "group-hover/service:scale-110"} transition-transform duration-200`}
          >
            <service.icon className='w-4 h-4 sm:w-5 sm:h-5 text-[#111316]' />
          </div>
        </div>
        <div className='flex-1'>
          <h4 className='text-white font-semibold text-base sm:text-lg group-hover/service:text-[#E7FF1A] transition-colors duration-200'>
            {service.name}
          </h4>
  ...
  ({
    ...
    return (
      ...
      >
        ...
        />
        <div
          className={`relative z-10 flex flex-col rounded-3xl border border-white/10 bg-white/5 backdrop-blur-xl shadow-2xl transition-all duration-300 p-4 sm:p-6 ${isExpanded ? "sm:p-8" : ""} hover:bg-white/10 hover:border-white/20`}
        >
          <div className='flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4'>
            <div className='flex items-center gap-4 sm:gap-6'>
              ...
              />
              <div className='flex-1'>
                <h3 className='text-xl sm:text-2xl md:text-3xl font-bold text-white group-hover:text-[#E7FF1A] transition-colors duration-200'>
                  {category.title}
                </h3>
                <p className='text-white/80 text-sm md:text-base mt-1 sm:mt-2 leading-relaxed'>
            ...
            <motion.div
              ...
              className={`p-2 rounded-xl bg-gradient-to-r ${category.color} self-end sm:self-center`}
            >
              <ChevronDown className='w-5 h-5 text-[#111316]' />
            </motion.div>
          </div>
          <AnimatePresence initial={false}>
            {isExpanded && (
              <motion.div
                ...
                className='overflow-hidden'
              >
                <div className='space-y-3 sm:space-y-4 mt-4 sm:mt-6 pt-4 sm:pt-6 border-t border-white/10'>
                  {category.services.map((service, index) => (
                    <motion.div
                      key={service.name}
                      initial={
                  ...
                  >
                    <motion.button
                      className={`group/btn inline-flex items-center gap-3 bg-gradient-to-r ${category.color} text-[#111316] font-bold uppercase py-3 px-6 sm:py-4 sm:px-8 rounded-2xl transition-all duration-200 hover:shadow-lg hover:shadow-[#E7FF1A]/20`}
                      whileHover={shouldReduceMotion ? {} : { scale: 1.02 }}
                      whileTap={shouldReduceMotion ? {} : { scale: 0.98 }}
                    >
                      Get Started
...
export function ServicesSection() {
  ...
  return (
    <section
      id='services'
      className='relative w-full bg-[#111316] py-16 sm:py-20 lg:py-24 overflow-hidden'
    >
      <div className='absolute inset-0 z-0'>
        <div className='absolute inset-0 bg-[linear-gradient(to_right,#80808012_1px,transparent_1px),linear-gradient(to_bottom,#80808012_1px,transparent_1px)] bg-[size:2rem_2rem] opacity-30' />
        {!shouldReduceMotion && (
      ...
      <div className='container mx-auto px-4 md:px-8 relative z-10'>
        ...
        >
          ...
          >
            <motion.div
              variants={itemVariants}
              className='inline-flex items-center gap-2 bg-white/10 backdrop-blur-xl border border-white/20 rounded-full px-4 py-2 mb-4'
            >
              <Sparkles className='w-4 h-4 text-[#E7FF1A]' />
              <span className='text-sm font-medium text-white/90'>
                Our Services
            ...
            {/* CORRECTED: Added a   
 for responsive line break */}
            <h2 className='font-bold text-[clamp(2.25rem,7vw,4rem)] leading-tight text-white mb-4'>
              COMPREHENSIVE
              <span className='bg-gradient-to-r from-[#E7FF1A] via-violet-400 to-cyan-400 bg-clip-text text-transparent'>
                SOLUTIONS
              </span>
            </h2>
            <p className='text-[clamp(1rem,4vw,1.25rem)] leading-relaxed text-white/80 max-w-3xl mx-auto'>
              From concept to completion, we offer a full spectrum of digital
              services designed to elevate your brand and drive growth.
            </p>
          </motion.div>
