...
const ProjectCard = ({ project }: { project: (typeof projectsData)[0] }) => {
  ...
  return (
    <motion.div
      layoutId={`card-${project.id}`}
      initial={
        shouldReduceMotion ? { opacity: 0 } : { opacity: 0, scale: 0.8, y: 30 }
      }
      animate={
        shouldReduceMotion ? { opacity: 1 } : { opacity: 1, scale: 1, y: 0 }
      }
    ...
    >
      ...
      <div className='relative z-10 flex h-full flex-col justify-end p-4 sm:p-6'>
        <div className='flex items-center gap-3 sm:gap-4'>
          ...
          <div>
            <h3 className='text-xl sm:text-2xl font-bold text-white group-hover:text-[#E7FF1A] transition-colors'>
              {project.title}
            </h3>
      ...
      <div className='container mx-auto px-4 md:px-8 lg:px-12 relative z-10'>
        ...
        >
          ...
          >
            ...
            {/* Stats Section */}
            <div className='flex flex-wrap justify-center gap-4 sm:gap-6 md:gap-8 2xl:gap-6 mb-6 sm:mb-8 lg:mb-12 2xl:mb-8'>
              {[
                { number: "50+", label: "Projects Completed" },
                { number: "4", label: "Categories" },
                { number: "100%", label: "Client Satisfaction" },
          ...
          {/* Projects Grid */}
          <motion.div className='grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-6 lg:gap-8 2xl:gap-6 mb-8 sm:mb-12 lg:mb-16 2xl:mb-12'>
            <AnimatePresence mode='sync'>
              {filteredProjects.map((project) => (
                <ProjectCard key={project.id} project={project} />
              ))}
