...
const ProjectCard = ({
  ...
  return (
    <motion.div
      ...
      animate='visible'
      exit='exit'
      className='group relative h-[350px] sm:h-[400px] w-full overflow-hidden rounded-3xl border border-white/10 bg-white/5 shadow-2xl backdrop-blur-xl hover:bg-white/10 hover:border-white/20 transition-all duration-300'
      whileHover={
        shouldReduceMotion
          ? {}
          : {
      ...
      <div className='relative z-10 flex h-full flex-col justify-end p-4 sm:p-6'>
        ...
        >
          <motion.div
            ...
            transition={{ type: "spring", stiffness: 400, damping: 25 }}
          >
            <Icon className='h-5 w-5 sm:h-6  text-[#111316]' />
          </motion.div>
          <div>
            <h3 className='text-lg sm:text-xl font-bold text-white group-hover:text-[#E7FF1A] transition-colors'>
              {project.title}
            </h3>
          ...
          >
            {categories.map((category) => (
              <motion.button
                key={category}
                onClick={() => setActiveCategory(category)}
                className={`relative rounded-full px-4 py-2 sm:px-5 sm:py-2.5 text-xs sm:text-sm font-medium transition-all duration-300 ${activeCategory === category ? "text-[#111316]" : "text-white/70 hover:text-white bg-white/5 border border-white/10 hover:bg-white/10"}`}
                whileHover={shouldReduceMotion ? {} : { scale: 1.05, y: -2 }}
