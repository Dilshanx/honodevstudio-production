...
const HolographicIcon = React.memo(
  ({
    ...
    return (
      <motion.div
        whileHover={shouldReduceMotion ? {} : "hover"}
        className='relative w-14 h-14 sm:w-16 sm:h-16 grid place-items-center flex-shrink-0'
      >
        <div
          className={`p-3 rounded-xl sm:rounded-2xl bg-gradient-to-r ${gradient} ${shouldReduceMotion ? "" : "group-hover:scale-110"} transition-transform duration-200`}
        >
          <IconComponent className='w-5 h-5 sm:w-6 sm:h-6 text-[#111316]' />
        </div>
        {!shouldReduceMotion && (
          <motion.div
            className={`absolute inset-0 rounded-xl sm:rounded-2xl bg-gradient-to-r ${gradient} opacity-20 blur-lg`}
            variants={{ hover: { scale: 1.2, opacity: 0.4 } }}
            transition={{
              type: "spring",
              damping: 15,
...
const ProcessCard = ({ step, index }: { step: ProcessStep; index: number }) => {
  ...
  return (
    <motion.div
      ...
      className={`group relative flex items-start sm:items-center gap-4 sm:gap-6 lg:gap-8 ${isEven ? "sm:flex-row" : "sm:flex-row-reverse"}`}
    >
      <div
        className={`hidden md:block absolute inset-0 bg-gradient-to-r ${step.color} rounded-3xl opacity-0 group-hover:opacity-10 transition-opacity duration-300 blur-xl`}
      />
      ...
      >
        <div className='relative p-4 sm:p-6 rounded-2xl sm:rounded-3xl bg-white/5 backdrop-blur-xl border border-white/10 hover:bg-white/10 hover:border-white/20 transition-all duration-300'>
          <h3 className='text-lg sm:text-xl lg:text-2xl font-bold text-white mb-2 group-hover:text-[#E7FF1A] transition-colors duration-200'>
            {step.title}
          </h3>
          <p className='text-sm sm:text-base text-white/80 leading-relaxed'>
...
export function OurProcess() {
 ...
 for responsive line break */}
          ...
          <div className='relative'>
            ...
            <div className='sm:hidden absolute left-7 top-0 bottom-0 w-1 bg-gradient-to-b from-transparent via-violet-400/40 to-transparent' />
            <div className='space-y-8 sm:space-y-12 lg:space-y-16'>
              {processSteps.map((step, index) => (
                <div key={index} className='relative sm:flex sm:justify-center'>
                  <div className='hidden sm:block absolute left-1/2 top-7 w-6 h-6 bg-gradient-to-r from-[#E7FF1A] to-violet-400 rounded-full border-2 border-white/50 shadow-lg -translate-x-1/2 z-10' />
                  <div
